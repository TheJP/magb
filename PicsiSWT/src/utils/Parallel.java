...
public class Parallel {
	...
	/**
	 * Parallel for: executes the loopBody for each int value in the semi-open range [start, stop)
	 * and performs a reduction in result.
	 * @param start < stop
	 * @param stop
	 * @param result
	 * @param creator initializes temporary reduction data
	 * @param loopBody
	 * @param reducer computes reduction of temporary reduction data and stores it in result
	 */
	public static <T> void For(int start, int stop, T result, final RedDataCreator<T> creator, final RedLoopBody<T> loopBody, final Reducer<T> reducer) {
		For(start, stop, 1, result, creator, loopBody, reducer);
	}
	 ...
	 * @param reducer computes reduction of temporary reduction data and stores it in result
	 */
	public static <T> void For(int start, int stop, int delta, T result, final RedDataCreator<T> creator, final RedLoopBody<T> loopBody, final Reducer<T> reducer) {
		assert delta != 0 : "delta must be not 0: " + delta;
